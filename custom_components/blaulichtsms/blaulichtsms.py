...
class BlaulichtSmsController:
    ...
    async def get_session(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.base_url}login", json=content) as r:
    ...
    async def get_alarms(self) -> list[dict]:
        ...
        try:
            ...
            self.logger.error("Failed to request blaulichtSMS alarms. Maybe there is no internet connection. %s", e)
            return None
    ...
    async def get_anonymized_alarms(self):
        alarms = await self.get_alarms()
        for a in alarms:
            a['recipients'] = len(list(filter(lambda r: (r.get("participation") == 'yes'), a['recipients'])))
